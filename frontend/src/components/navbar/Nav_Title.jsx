...
export default function Nav_Title(props) {
    ...
    if(props.mode==false)
    {
        element = <DarkModeOutlinedIcon />
    }
    else{
        element = <LightModeOutlinedIcon />
    }
    ...
    return (
        <div className=" flex justify-center sm:justify-between gap-x-5 gap-y-5 w-full sm:w-3/5 grow flex-wrap content-center items-center ">
            ...
            <Button className="w-16 h-14 lg:h-14 bg-[#f7f9fa] rounded-md shadow-none hover:shadow-none  px-3 py-3 flex justify-center items-center shrink dark:bg-[#1e293b] dark:text-white text-[#0f172af9]" onClick={props.setDarkMode}>{element}</Button>
        </div>
    )
}
